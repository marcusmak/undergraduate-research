...
public class LabelDisplayView extends FrameLayout {
    ...
    public void addDot(final float absX, final float absY, final float scale,boolean gpsMode) {
        Dot dot = new Dot(getContext(),gpsMode);
        dot.absCoord = new PointF(absX, absY);
        dot.canvasCoord = new PointF(absX / scale, absY / scale);
        if (gpsMode)
            dot.setImageResource(R.drawable.gps_mode_dot);
        else
    ...
    public void cancelFocusedDot() {
        if (hasFocusedDot()) {
            if (focused.gpsMode)
                focused.setImageResource(R.drawable.gps_mode_dot);
            else
    ...
    public void deleteAllDots() {
        Iterator<Dot> dot = dotList.iterator();
        while (dot.hasNext()) {
            Dot temp = dot.next();
            if (!(temp.gpsMode)){
                removeView(temp);
    ...
    private class Dot extends ImageView {
        ...
        public Dot(Context context, boolean gpsMode) {
            super(context);
            this.gpsMode = gpsMode;
        }

        public Dot(Context context, AttributeSet attrs,boolean gpsMode) {
            super(context, attrs);
            this.gpsMode = gpsMode;
        }
    }
